...
const MIN_MBPS = 2;
const STUDENT_EXAM_ID_KEY = "student_exam_id";
...
function StudentTechnicalReadinessCheckContent() {
  ...
  // Get examId from query param or localStorage
  React.useEffect(() => {
    const fromQuery = searchParams?.get("examId");
    if (fromQuery) {
      setExamId(fromQuery);
      try {
        localStorage.setItem(STUDENT_EXAM_ID_KEY, fromQuery);
      } catch {
        /* ignore */
      }
    } else {
      try {
        const stored = localStorage.getItem(STUDENT_EXAM_ID_KEY);
        if (stored) setExamId(stored);
      } catch {
        /* ignore */
      }
    }
  ...
  // Run camera, mic, internet on mount
  React.useEffect(() => {
    runCameraCheck();
    runMicrophoneCheck();
    runInternetCheck();
  ...
  const handleStartExam = () => {
    const id = examId || (typeof window !== "undefined" ? localStorage.getItem(STUDENT_EXAM_ID_KEY) : null);
    if (id) {
      router.push(`/student/exams/${id}/exam-taking`);
    } else {
  ...
  return (
    <div className="space-y-6">
            ...
            <li key={check.id} className="flex items-center gap-4 px-4 py-4 sm:px-6">
              <div
                className={cn(
                  ...
                  check.status === "running" && "bg-amber-100 text-amber-700",
              ...
              >
                ...
                {check.status === "running" && (
                  <span className="h-6 w-6 animate-spin rounded-full border-2 border-amber-500 border-t-transparent" aria-hidden />
                )}
                {check.status === "idle" && <span className="text-2xl font-bold">?</span>}
              </div>
              <div className="min-w-0 flex-1">
                ...
                <p
                  className={cn(
                    ...
                    (check.status === "running" || check.status === "idle") && "text-slate-500"
                  )}
                >
                  {check.detail}
                </p>
                {check.id === "microphone" && check.status === "running" && audioLevel >= 0 && (
                  <div className="mt-2 h-1.5 w-full max-w-[120px] overflow-hidden rounded-full bg-slate-200">
                    <div className="h-full rounded-full bg-amber-500 transition-all duration-150" style={{ width: `${audioLevel}%` }} />
                  </div>
                )}
              </div>
              {(check.status === "fail" || check.status === "idle") && (
                <Button type="button" variant="outline" size="sm" onClick={check.onRetry}>
        ...
        {checks[4].status === "idle" && (
          <div className="border-t border-slate-100 bg-amber-50/50 px-4 py-3 sm:px-6">
            <p className="text-sm text-slate-700">
              Click &quot;Test&quot; next to Screen sharing and choose a window or your full screen when prompted.
            </p>
          </div>
        )}
      ...
      <p className="text-center text-sm text-slate-500">
        <Link href="/student/exams" className="font-medium text-sky-600 hover:underline">
          Exam center
        </Link>
        {" · "}
        <Link href="/student" className="font-medium text-sky-600 hover:underline">
          Dashboard
        </Link>
      </p>
    </div>
