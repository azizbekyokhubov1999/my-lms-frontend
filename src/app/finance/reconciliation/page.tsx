...
export default function ReconciliationPage() {
  ...
  const unmatchedLms = MOCK_LMS.filter((p) => !p.matchedId);
  const unmatched1c = MOCK_1C.filter((r) => !MOCK_LMS.some((p) => p.matchedId === r.id));
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={cn(
                        ...
                        p.matchedId ? "text-emerald-700" : "text-amber-700",
                      )}
                    >
                      {p.matchedId ? "Matched" : "Unmatched"}
                    </span>
                  </div>
                </div>
        ...
        <Card className="p-4">
          ...
          <ul className="mt-3 space-y-2">
            {MOCK_1C.map((r) => {
              const isMatched = MOCK_LMS.some((p) => p.matchedId === r.id);
              return (
                <li
                  key={r.id}
                  className={cn(
      ...
      <div className="flex flex-wrap gap-4 rounded-lg border border-slate-200 bg-slate-50/50 p-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-600">Matched: {MOCK_LMS.filter((p) => p.matchedId).length} LMS, {MOCK_1C.filter((r) => MOCK_LMS.some((p) => p.matchedId === r.id)).length} 1C</span>
        </div>
        <div className="flex items-center gap-2">
