...
export default function IncidentDetailPage() {
  ...
  }, [incident, persist, rootCause, summary]);
          ...
          <Card className="border-indigo-400/30 bg-slate-950">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-3 text-lg font-semibold text-slate-100">{incident.title}</p>
                <p className="mt-1 text-sm text-slate-100/70">Service: {incident.service}</p>
          ...
          <Card className="border-indigo-400/30 bg-slate-950">
            ...
            <div className="mt-4 space-y-3">
              {incident.timeline.map((ev, idx) => (
                <div key={`${ev.ts}-${idx}`} className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-100/90">{ev.label}</p>
                    <p className="mt-1 text-sm text-slate-100/70">{ev.detail}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {incident.postMortem ? (
              <div className="mt-6 rounded-xl border border-indigo-400/30 bg-indigo-400/5 p-4">
                <p className="text-sm font-semibold text-slate-100">Post-mortem saved</p>
